...
description: >
  Refactor a method to reduce its cognitive complexity by extracting focused helper methods.
  Use when a method has high cyclomatic or cognitive complexity, deeply nested conditionals,
  long if-else chains, or when code analysis tools flag complexity warnings. Also use when
  the user wants to break down a large function, improve code readability, or extract logic
  into smaller focused methods while preserving behavior.
---

...
name: reference-core
description: >
  Explains the mental model and architecture of Angular's `packages/core`. Use any time you
  plan to work with code in `packages/core`, including the rendering pipeline (Ivy/Render3),
  dependency injection system, change detection, signals, or component lifecycle. Also use
  when debugging Angular core internals, understanding LView/TView structures, or modifying
